...
## 1. Nếu chưa biết Cloudflare Tunnel và cloudflared là gì

Bình thường, để truy cập Grafana từ Internet, bạn phải mở port trên VPS:

```text
Browser -> public IP của Central VPS -> port 3000 -> Grafana
```

Mô hình này không phù hợp cho Grafana production vì Grafana bị lộ trực tiếp ra Internet.

Cloudflare Tunnel đổi hướng kết nối:

```text
cloudflared trên Central VPS -> mở kết nối outbound tới Cloudflare
Browser admin -> Cloudflare Edge -> đi ngược qua tunnel đó -> Grafana nội bộ
```

Các vai trò cần tách rõ:

| Thành phần | Làm gì |
|---|---|
| Cloudflare Edge | Nhận request từ Internet cho domain Grafana |
| Cloudflare Tunnel | Đường nối từ Cloudflare về Central VPS |
| `cloudflared` | Container chạy trên Central, giữ tunnel và gọi `http://grafana:3000` |
| Cloudflare Access | Lớp xác thực người được vào Grafana |

Tunnel lo đường mạng. Access lo quyền truy cập. `cloudflared` là process thực sự chạy trong stack để giữ tunnel.
...
## 6. Chuẩn bị Grafana cho domain public

Cloudflare Tunnel đã route domain public về `cloudflared`. Nhưng Grafana vẫn chạy bên trong Docker bằng URL nội bộ `http://grafana:3000`.

Nên Grafana cần biết URL public để khi nó tự tạo link/redirect/callback, nó tạo đúng domain public. Biến quan trọng nhất là `GF_SERVER_ROOT_URL`. `GF_SERVER_DOMAIN` chỉ dùng để Grafana kiểm tra Host header khi bật `GF_SERVER_ENFORCE_DOMAIN=true`; nó không làm Cloudflare route domain.
...
Nếu chỉ test nhanh, tunnel có thể chạy khi thiếu `GF_SERVER_DOMAIN`. Với production, nên set đủ 3 biến trên để tránh redirect sai domain và chặn Host header lạ.
...
## 7. Tạo tunnel trên Cloudflare Dashboard
...
## 9. Bật Cloudflare Access cho Grafana
...
- repo hiện đang pin `cloudflare/cloudflared:2026.3.0`
...
## 16. Vận hành production

### 16.1. Pin version

Cho production:

- pin `cloudflare/cloudflared:2026.3.0` hoặc version mới hơn mà bạn đã test
...
- [ ] Nếu bật `GF_SERVER_ENFORCE_DOMAIN=true`, `GF_SERVER_DOMAIN` đúng với domain thật
